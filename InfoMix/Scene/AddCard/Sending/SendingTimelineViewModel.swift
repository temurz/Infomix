...
extension SendingTimelineViewModel: ViewModel {
    ...
    func transform(_ input: Input, cancelBag: CancelBag) -> Output {
        ...
        serialCardId.handleEvents(receiveOutput: {
            ...
            let input = SerialNumbersInput(serialCardId: serialCardId, serialNumbers: self.cardConfig.serialNumbers(separator: ","))
        ...
        isSendingSerialNumbers.handleEvents(receiveOutput: {
            ...
            if isLoading {
                ...
                if let index = output.timelines.lastIndex(where: { it in
                    ...
                    let timeline = output.timelines[index].clone(status: .loading, title: "Sending serial numbers...".localized(), self.cardConfig.serialNumbers(separator: "\n"))
        ...
        isResendingSerialNumbers.handleEvents(receiveOutput: {
            ...
            if isResendingSerialNumbers {
                ...
                if let index = output.timelines.lastIndex(where: { it in
                    ...
                    let timeline = output.timelines[index].clone(status: .loading, title: "Resending serial numbers...".localized(), self.cardConfig.serialNumbers(separator: "\n"))
        ...
        serialCard.handleEvents(receiveOutput: {
            ...
            if let index = output.timelines.lastIndex(where: { it in
                ...
                let timeline = output.timelines[index].clone(status: .success, title: "Sended serial numbers".localized(), self.cardConfig.serialNumbers(separator: "\n"))
                output.timelines[index] = timeline
                
                
                let input = AdditionalDataInput(serialCardId: sc.id, installedDate: self.cardConfig.installedDate, phone: cardConfig.getPhones(), latitude: nil, longitude: nil)
        ...
        transaction.handleEvents(receiveOutput: {
            ...
            if let index = output.timelines.lastIndex(where: { it in
                ...
                let timeline = output.timelines[index].clone(status: .success, title: "Calculated bonuses".localized())
                output.timelines[index] = timeline
            }
                ...
                if let lastActionIndex = output.timelines.lastIndex(where: { it in
                    ...
                    if let input = lastTimeline.input {
                        if input is SerialNumbersInput {
                            ...
                            output.timelines.append(SendingTimeline(id: SendingTimelineId.serialNumbers.rawValue, status: .loading, title: "Waiting...".localized(), content: self.cardConfig.serialNumbers(separator: "\n"), date: Date(), input: input, inDispute: true))
