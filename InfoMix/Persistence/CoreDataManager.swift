...
class CoreDataManager: NSObject {
    ...
    lazy var persistentContainer: NSPersistentContainer = {
         ...
         */
        let container = NSPersistentCloudKitContainer(name: "AppPersistence")
    ...
    func fetchObject<T: NSManagedObject>(by id: Int32) -> T? {
    ...
    func fetchObject<T: NSManagedObject>(where predicates: NSCompoundPredicate? = nil) -> T? {
        let result: [T]? = self.fecth(where: predicates)
        return result?.first
    }
    ...
    func count<Entity>(entity: Entity.Type, where predicates: NSCompoundPredicate? = nil) -> Int  where Entity : NSManagedObject {
        let context = persistentContainer.viewContext
